...
<body>
        ...
        <table class="table table-striped table-hover">
            ...
            <tbody>
                <?php foreach($patients as $each){ ?>
                    <?php $tenBacSi = $patientService->getNameDoctor($each); ?>
                    <tr>
                        <td><?=$each->getId();?></td>
                        ...
                        <td>
                            <a onclick="return Del('<?=$each->getId()?>')" href="?controller=student&action=delete&id=<?=$each->getId()?>">
    ...
    <script>
        function Del(name){
            return confirm("Bạn chắc chắn muốn xoá bệnh nhân có mã là: " + name + "?");
        }
    </script>
</body>
</html>